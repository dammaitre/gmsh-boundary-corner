"""
standard.py — pipeline-scale BoundaryDoubleCorner case, far-field domain
sized in multiples of the body length L (= 2A): 10L upstream, 20L downstream,
25L in height.

Same body geometry and BL parameters as bigdoublecorner.py (A=100m, B=27m),
the body being a SINGLE spline (nose → top → tail) covered by one
BoundaryDoubleCorner field. Only the far-field domain extents (and LC_FAR,
scaled with them) differ from bigdoublecorner.py.

Usage:
    python standard.py           # headless, mesh + checkMesh only
    python standard.py --gui     # open the mesh in the gmsh GUI
    python standard.py --of      # also run foamRun after checkMesh
"""
...
# ── Mesh sizes ────────────────────────────────────────────────────────────────
# Far-field cell size scaled with the domain so triangle density stays
# similar to bigdoublecorner.py (there: LC_FAR=20.0, Y_FAR=150.0).
...
# ── Output ────────────────────────────────────────────────────────────────────
CASE_DIR = os.path.join(_root, "testing", "standard_of")
MAX_TIME = 200
...
# x is sampled directly over [-A, A] (no angle parameter), clustered near
# the nose/tail (±A) by a 1-cos map; y is the explicit ellipse function of x.
